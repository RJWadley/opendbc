...
Why simulate the ESP?
We could add tests that assert specific signal combinations,
but the ESP is a complex state machine and behavior can *very* easily
go wrong in unexpected ways even if everything seems correct from the surface.
"""
...
STEP_DT = DT_CTRL * CarControllerParams.ACC_CONTROL_STEP
ACC_ANHALTEWEG_NEUTRAL = 20.46
ESP_HOLD_ACQUIRE_SPEED_MS = 0.33 / 3.6  # 0.33 km/h, matches observed threshold
ESP_HOLD_TIMER_LIMIT_FRAMES = 65
# in real life, this timeout depends on road grade
HILL_DECEL_TIMEOUT_FRAMES = 120
# amount of haltemoment torque needed to cycle
...
class ESPState:
  ...
  faulted: bool = False
  # counts hold long hold has been confirmed since last reset.
  # resets when wheel impulse while no hold confirmed OR
  # successful conditional release of a hold confirmation.
...
@dataclass
class SimInputs:
  acc_anfahren: bool = False
  acc_anhalten: bool = False
  acc_anhalteweg: float = ACC_ANHALTEWEG_NEUTRAL
  acc_sollbeschl_06: float = 0.0


class ESPTSKSimulator:
  """
  Callers set `speed_ms` and `esp_hold_torque_nm` to reflect the
